...
- **Status Effect Soft Cap:** Even when a card clears the quality bar above, avoid letting status effects dominate a subdeck. As a guideline, no more than roughly 25% of cards in a subdeck should apply, grant, remove, or otherwise interact with status effects. Before proposing any card whose mechanic involves a status effect, check how many status-effect cards already exist in the subdeck. If the deck is already heavy with status effects, you **must** choose a completely different non-status mechanic.
- Do not default to common status effects out of habit. Be creative and create unique mechanics.
- If a status effect is the best fit as a *secondary* component, write a one-sentence justification explaining why it serves the card's lore and gameplay identity better than a non-status mechanic, then verify the subdeck is still at a reasonable density before proceeding.
