#!/usr/bin/env python3
"""Generate a full-body T-pose character reference image in a single gpt-image-2 edit call.

Unlike new_image's generate-then-colorize pipeline, this skill has plenty of
shipped character art to draw on already, so it skips the throwaway
...
T_POSE_STANCE = (
    ...
    "straight out to the sides at shoulder height forming a T shape, head "
    "level and facing forward with a neutral expression. This is a "
    "rigging/reference pose, not an action pose. "
    "The pose and body MUST be completely symmetric left-to-right: a "
    "mirror-image left and right side, identical arm angle and height on "
    "both sides, identical leg stance on both sides, head perfectly centered "
    "and not tilted, both hands posed identically as mirror images of each "
    "other. Treat the vertical centerline of the character as a perfect "
    "mirror axis. "
    "Both hands are semi-closed — fingers gently curled inward as in a "
    "relaxed rigging pose, not stiff and flat, not fully open with spread "
    "fingers, and not a tight fist. "
    "The character's entire body, from the top of the head to the soles of "
    "the feet, must be fully visible, centered in frame, and not cropped, "
    "with even empty margin above the head and below the feet. "
    "No ground, no props unless they are worn/attached to the body, no "
    "ground shadow, and no other characters, so the figure reads cleanly as "
    "a single isolated animation-reference subject."
)
...
STYLE_DIRECTION = (
    ...
    "expressive slightly cartooned anatomy, bold dark ink outlines, "
    "flat-to-soft cel shading, painterly watercolor-like texture, moody "
    "magical lighting, and a retro illustrated fantasy atmosphere. Make it "
    "feel like an old animated fantasy character model sheet, not realistic "
    "modern concept art, not glossy, not photoreal, not 3D, and not anime. "
    "Avoid AI-generated anatomy mistakes such as extra fingers, double "
    "hands, duplicate limbs, or distorted faces. Avoid a flat sepia or "
    "uniformly brown color cast; use richer greens, blues, reds, golds, and "
    "earth tones as appropriate to the character."
)
...
REFERENCE_USAGE = (
    ...
    "poses, outfits, subjects, or compositions; this is a new, different "
    "character rendered in the strict T-pose described above."
)

GENERATE_PROMPT = (
    "Create a tall 2:3 portrait-format full-body character reference sheet "
    "illustration for a RetroLOTR game character.\n"
    f"{T_POSE_STANCE}\n"
    f"{BACKGROUND_REQUIREMENT}\n"
    f"{STYLE_DIRECTION}\n"
    f"{REFERENCE_USAGE}\n"
    "No modern UI elements, no text overlays, no logos, no card frame, no "
    "white border, no watermarks, no second character, no scenery or props "
    "in the background — only the flat chroma-key color."
)
...
def validate_size(size: str) -> None:
    ...
    if not match:
        die("size must be WIDTHxHEIGHT (e.g. 640x1024) or auto.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a full-body T-pose character reference image in one gpt-image-2 edit call"
    )
    parser.add_argument("--name", required=True, help="Character name")
    parser.add_argument(
        "--description",
        required=True,
        help="Character description: race/species, gear, build, notable visual traits, alignment/mood",
    )
    parser.add_argument("--out", help="Path to write the generated image (default: Assets/Art/Characters/Portraits/<Name>.png)")
...
def build_prompt(name: str, description: str) -> str:
    header = (
        f"Character name: {humanize_name(name)}.\n"
        f"Character description: {description.strip()}\n"
        "Ensure the final image clearly matches this named character and description.\n"
    )
...
def build_bg_mask(arr, bg_color, tolerance: float):
    ...
    so background-colored patches inside the character silhouette survive.
    """
    import numpy as np
...
def dilate_mask(mask, iterations: int):
    ...
    anti-aliased edge pixels (blended between the character's ink outline and
...
def ensure_transparent_background(image_path: Path, tolerance: float = BACKGROUND_KEY_TOLERANCE) -> str:
    ...
    for spritesheet-generation). This is expected to run on every image, not
    just as a rare fallback.
    ...
    # Pockets of chroma-key color enclosed by the character's own silhouette
    # (e.g. between splayed fingers, between crossed limbs) aren't reachable
    # from the border by the flood-fill above, so key them out directly by
    # color match too — safe here because the RetroLOTR palette never
    # legitimately uses the chroma-key color.
    import numpy as np
...
def main() -> int:
    ...
    if args.dry_run:
        print("gpt-image-2 single-call T-pose character generation dry-run")
    ...
    print(f"Character name: {humanize_name(args.name)}")
    print(f"Character description: {args.description}")
    print("Prompt used:")
    print(prompt)
    return 0
