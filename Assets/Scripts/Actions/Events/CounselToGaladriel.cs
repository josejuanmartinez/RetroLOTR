...
public class CounselToGaladriel : CharacterAction
{
    ...
    public override void Initialize(Character c, Func<Character, bool> condition = null, Func<Character, bool> effect = null, Func<Character, Task<bool>> asyncEffect = null)
    {
        ...
        async Task<bool> counselAsync(Character character)
        {
            ...
            {
                target = allies.OrderByDescending(ch => ch.GetMage()).FirstOrDefault();
            }

            if (target == null) return false;

            Leader owner = character.GetOwner();
            List<Hex> radiusHexes = character.hex.GetHexesInRadius(3);
            character.hex.RevealArea(3, true, owner);
            ...
            target.ApplyStatusEffect(StatusEffectEnum.ArcaneInsight, 2);

            MessageDisplayNoUI.ShowMessage(character.hex, character, $"Counsel to Galadriel reveals radius 3 and grants {target.characterName} Arcane Insight (2).", Color.cyan);
            return true;
        }
