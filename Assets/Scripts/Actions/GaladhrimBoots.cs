...
public class GaladhrimBoots : CharacterAction
{
    ...
    public override void Initialize(Character c, Func<Character, bool> condition = null, Func<Character, bool> effect = null, Func<Character, Task<bool>> asyncEffect = null)
    {
        ...
        async Task<bool> bootsAsync(Character character)
        {
            ...
            {
                target = allies.FirstOrDefault(ch => ch.HasStatusEffect(StatusEffectEnum.Fear))
            ...
            bool clearedFear = false;
            if (target.hex != null && target.hex.terrainType == TerrainEnum.forest && target.HasStatusEffect(StatusEffectEnum.Fear))
            {
                target.ClearStatusEffect(StatusEffectEnum.Fear);
                clearedFear = true;
            }

            string fearText = clearedFear ? " Fear removed on forest terrain." : string.Empty;
            MessageDisplayNoUI.ShowMessage(character.hex, character, $"{target.characterName} gains Haste (1) and Hidden (1).{fearText}", Color.cyan);
            return true;
        }

        base.Initialize(c, condition, effect, bootsAsync);
    }
}
