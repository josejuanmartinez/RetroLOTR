...
public class IthilienRangerHood : CharacterAction
{
    ...
    public override void Initialize(Character c, Func<Character, bool> condition = null, Func<Character, bool> effect = null, Func<Character, Task<bool>> asyncEffect = null)
    {
        ...
        async Task<bool> hoodAsync(Character character)
        {
            ...
            List<Hex> radiusHexes = character.hex.GetHexesInRadius(1);
            character.hex.RevealArea(1, true, owner);
            owner?.AddTemporarySeenHexes(radiusHexes);
            owner?.AddTemporaryScoutCenters(new[] { character.hex });

            List<string> enemyNames = radiusHexes
                .Where(hex => hex != null && hex.characters != null)
                .SelectMany(hex => hex.characters)
                .Where(ch => ch != null && !ch.killed && ch.GetAlignment() != character.GetAlignment() && !ch.IsHidden())
            ...
            string scoutText = enemyNames.Count > 0 ? " Nearby enemies revealed." : string.Empty;
            MessageDisplayNoUI.ShowMessage(character.hex, character, $"{target.characterName} gains Hidden (1).{scoutText}", Color.cyan);

            if (!isAI && enemyNames.Count > 0)
            {
                ...
                {
                    MessageDisplayNoUI.ShowMessage(character.hex, character, enemyName, Color.yellow);
                    while (MessageDisplayNoUI.IsBusy())
                    {
                        await Task.Yield();
                    }
