...
public class DestroyPort : CommanderEnemyPCAction
{
        ...
        Func<Character, System.Threading.Tasks.Task<bool>> asyncEffect = null)
    {
        ...
        effect = (actor) =>
        {
            ...
            MessageDisplayNoUI.ShowMessage(pc.hex, actor, $"{pc.pcName} port destroyed!", Color.red);
            pc.hex.RedrawPC();
            pc.hex.RedrawArmies();
        ...
        condition = (actor) =>
        {
            ...
            PC pc = actor.hex.GetPC();
            return pc != null && pc.hasPort && actor.GetCommander() >= 3;
        };
